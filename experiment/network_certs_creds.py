...
def wpa(node, conf=None):
    conf = conf if conf else str(node)
    command = 'wpa_supplicant -i ' + str(node.intf()) + ' -D wired'
    config = '-c ' + IEDS_ROOT + conf + '.conf -dd -f'
...
def main(mac_address, interface):
    ...
    ied1.cmd('experiment/ieds/./server_ied_sub', ied1.intf(), '&')
    ied2.cmd('experiment/ieds/./server_ied_sub', ied2.intf(), '&')

    wpa(ied1)
    sleep(10)
    wpa(ied2, 'ied2_credentials')
