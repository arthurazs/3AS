...
def load(evs, rep, verbose=False):
    ...
    authentication = authentication.query(
        'Traffic != "OpenFlow" or (Traffic == "OpenFlow" and information.str.contains("Type: "))')
    while not authentication.empty:
        try:
            helper = authentication.query('Traffic == "EAPoL" and information == "Start"').values[0][-1]
        except IndexError:
            helper = authentication.values[-1][-1]
        start = authentication.query('time < @helper').query(
            'Traffic == "OpenFlow" and information == "Type: OFPT_PACKET_IN"').values[0][-1]
        helper2 = authentication[authentication.Traffic == 'MMS'].values[0]
        end = helper2[-1]
        ip = helper2[0]
        authentication = authentication.query('source != @ip')
        this_rep['delay'] = [end - start]  # TODO media de todos veiculos no experimento + media por experimento? e se colocar a media separada por veiculo num grafico de pontos e tals
        authentication = authentication.query('time > @helper')

    # return partial
    # return partial.groupby('Traffic').sum()
    return this_rep.mean()
